...
const Consumer = ({content}) => {
  ...
  const handleClick = () => {
    navigate('/contact#contact-form', { state: { scrollToContact: true } });
  }
    ...
    <div className="bg-black __responsive_gapY">
      <div className="__container md:col-span-2 w-full p-6 __gradient rounded-2xl shadow-2xl text-white">
        <div className=" inline-flex items-center gap-3 pb-3 __position_border __heading_gap pr-2">
          ...
          <h2 className="text-3xl font-bold __heading text-white">{content?.support_faq_page?.page_details?.title}</h2>
        </div>

        <div className="space-y-4">
          {content?.support_faq_page?.details?.map((faq, index) => (
            <div
              key={index}
        ...
        <div className="mt-10 flex flex-col items-center">
          <div className="relative flex items-center justify-between w-full">
            <Button varient={'outline'} className={'!p-0 !rounded-[50%]'}>
              <Link to='/support' className="p-3 block">
